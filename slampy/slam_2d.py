...
class Slam2D(Slam):
    ...
    def saveMidx(self):
        ...
        lines.append("<scale     x='%s' y='%s'>" % (
            cstring10(physic_box.size()[0] / logic_box.size()[0]),
            cstring10(physic_box.size()[1] / logic_box.size()[1])))
...
""")
    ...
    def findAllMatches(self, nthreads=8):
        ...
        if self.debug_mode:
            ...
            # nthreads needs to be one here if using cv2.cuda because cv2.cuda does not appear to be thread safe.
            # When not using GPU setting nthreads to 1 also speeds up the run considerably.
            results = RunJobsInParallel(jobs, advance_callback=lambda ndone: self.advanceAction(ndone), nthreads=1)
