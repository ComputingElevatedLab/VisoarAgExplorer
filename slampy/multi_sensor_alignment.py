...
class MultiSensorAlignment:
    ...
    def solveSlam(self, multi, midx_filename):
        multi = [
            single[:, :, 0] if len(single.shape) == 3 else single for single in multi
        ]
        ...
        slam.dtype = DType.fromString(
            "uint8"
        )  # note the images have been converted to uint8
        slam.calibration = self.calibration

        extractor = ExtractKeyPoints(
            self.min_keypoints, self.max_keypoints, self.anms, self.extractor_method
        )
            ...
            for camera1 in camera2.getAllLocalCameras():
                if camera1.id < camera2.id:
                    matches, H21, err = FindMatches(
                        ...
                        self.max_reproj_error * slam.width,
                        self.ratio_check,
                    )
                    matches = (
                        []
                    ...
                    camera2.getEdge(camera1).setMatches(
                        matches, err if err else str(len(matches))
                    )
        print(
            "Found num_matches(", num_matches, ") matches in ", t2.elapsedMsec(), "msec"
        )
        ...
        for C in range(nsensors):
            quad = Quad(
                slam.cameras[C].homography, Quad(multi[C].shape[1], multi[C].shape[0])
            )
            p0, p1, p2, p3 = (
                quad.getPoint(0),
                quad.getPoint(1),
        ...
        print(
            ...
            self.cropped_dimensions,
            "width",
            self.cropped_dimensions[2],
            "height",
            self.cropped_dimensions[3],
        )
        ...
        self.warp_matrices = [
            numpy.matmul(
                MatrixToNumPy(
                    Matrix(
                        1.0,
                        0.0,
                        float(-self.cropped_dimensions[0]),
                        0.0,
                        1.0,
                        float(-self.cropped_dimensions[1]),
                        0.0,
                        0.0,
                        1.0,
                    )
            ...
            for H in [slam.cameras[C].homography for C in range(nsensors)]
        ]
    ...
    def warpPerspective(self, single, C, capture_warp_matrices=None):
        ...
        if capture_warp_matrices is None:
            return cv2.warpPerspective(
                single, self.warp_matrices[C], (W, H), flags=cv2.INTER_LANCZOS4
            )
        else:
            return cv2.warpPerspective(
                single,
                capture_warp_matrices[C],
                (W, H),
        ...
        for I in range(nsensors):
            for J in range(I + 1, nsensors):
                SaveUint8Image(
                    ...
                    InterleaveChannels(
                        [
                            self.warpPerspective(multi[I], I),
                            self.warpPerspective(multi[J], J),
                        ]
                    ),
                )
    ...
    def doAlign(self, multi, capture_warp_matrices=None):
        if capture_warp_matrices is None:
            return [self.warpPerspective(single, i) for i, single in enumerate(multi)]
        else:
