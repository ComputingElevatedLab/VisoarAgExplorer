...
class ExtractKeyPoints:
    ...
    def __init__(
        ...
        if extractor_method == "akaze":
            ...
            print(
                f"Keypoint extraction method {extractor_method} not recognized, defaulting to akaze..."
            )
            self.detector = cv2.AKAZE.create()
    ...
    def doExtractAkaze(self, energy):
        ...
        if self.anms > 0 and len(keypoints) > self.anms:
            ...
            good_indices = KeyPoint.adaptiveNonMaximalSuppression(
                responses, xs, ys, self.anms
            )
